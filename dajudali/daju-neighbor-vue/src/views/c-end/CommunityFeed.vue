        ...
        <div v-for="p in posts" :key="p.id" class="post-card">
          ...
          <div class="post-actions">
            <button class="act-btn" :class="{ liked: p.liked_by_me }" @click="doLike(p)">
              <svg width="16" height="16" viewBox="0 0 24 24" :fill="p.liked_by_me ? '#FF7B2C' : 'none'" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
              {{ p.like_count || 0 }}
            </button>
...
.nav-link { font-size: 14px; color: #FF7B2C; }
.login-tip { padding: 60px 32px; text-align: center; color: #999; }
...
.topic-chip.active { background: #FF7B2C; border-color: #FF7B2C; color: #fff; }

.feed-list { padding: 0 16px; }
.post-card { background: #1a1a1a; border: 1px solid #2e2e2e; border-radius: 14px; padding: 14px; margin-bottom: 12px; }
.post-head { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
.post-avatar { width: 38px; height: 38px; border-radius: 50%; background: #C4923A; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 16px; font-weight: 600; flex-shrink: 0; }
.post-name { font-size: 14px; color: #eee; }
.post-time { font-size: 11px; color: #666; margin-top: 2px; }
.post-content { font-size: 15px; color: #f0f0f0; line-height: 1.6; word-break: break-word; }
...
.post-topic { color: #FF7B2C; font-size: 12px; }
.post-category { color: #999; font-size: 12px; background: #2e2e2e; padding: 2px 8px; border-radius: 10px; }
.post-actions { display: flex; gap: 20px; margin-top: 12px; padding-top: 10px; border-top: 1px solid #2e2e2e; }
.act-btn { display: flex; align-items: center; gap: 5px; background: none; border: none; color: #999; font-size: 13px; cursor: pointer; padding: 0; }
.act-btn.liked { color: #FF7B2C; }
.feed-empty { text-align: center; color: #888; padding: 40px 0; font-size: 14px; }

.fab { position: fixed; right: 20px; bottom: 80px; width: 52px; height: 52px; border-radius: 50%; border: none; background: linear-gradient(135deg, #FF7B2C, #E85D04); display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 4px 16px rgba(255,123,44,0.4); z-index: 10; }

.post-pop { padding: 20px 16px 30px; }
.pop-title { font-size: 16px; font-weight: 600; color: #fff; margin-bottom: 14px; }
.post-input { width: 100%; min-height: 100px; background: #1a1a1a; border: 1px solid #2e2e2e; border-radius: 10px; color: #eee; padding: 12px; font-size: 15px; resize: none; }
...
.comment-item { padding: 10px 0; border-bottom: 1px solid #2e2e2e; }
.comment-name { font-size: 13px; color: #C4923A; margin-bottom: 4px; }
.comment-content { font-size: 14px; color: #eee; line-height: 1.5; }
.comment-empty { text-align: center; color: #888; padding: 20px 0; }
.comment-input-row { display: flex; gap: 10px; }
.comment-input { flex: 1; background: #1a1a1a; border: 1px solid #2e2e2e; border-radius: 20px; color: #eee; padding: 10px 14px; font-size: 14px; }
.comment-send { padding: 10px 18px; border-radius: 20px; border: none; background: #FF7B2C; color: #fff; font-size: 14px; cursor: pointer; }
</style>
