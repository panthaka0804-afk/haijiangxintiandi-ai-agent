      ...
      <div v-for="(a, i) in decorated" :key="a.id" class="act-card" :style="{ '--ac-bg': a.bg, '--ac-bd': a.bd }" @click="$router.push(`/activities/${a.id}`)">
        ...
        <div class="act-info">
          ...
          <div class="act-info-meta">
            <span>时间 {{ a.start_date }}</span>
            <span>地点 {{ a.venue }}</span>
          </div>
        </div>
...
const FALLBACK = [
  ...
  { id: 3, title: '开学季文具特卖', desc: '开学焕新，文具图书低至5折，满199减50。', venue: '1F 中庭', start_date: '2026-09-01', end_date: '2026-09-03', gradient: 'linear-gradient(135deg, #C4923A, #FFB347)', status: 'upcoming' },
]
...
<style scoped>
.act-page { padding: 8px 12px; min-height: 100vh; background: #000000; }
