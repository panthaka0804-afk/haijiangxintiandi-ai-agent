  ...
  <div class="life-page">
    ...
    <div class="life-grid">
      <div
        ...
        :key="p.plan_type"
        class="life-card"
        :style="cardStyle(p.plan_type === 'monthly' ? 0 : 1)"
      >
        <div class="lc-top">
          <span class="lc-icon">{{ p.plan_type === 'monthly' ? '🅿️' : '🔌' }}</span>
    ...
    <div class="life-card ns-card" :style="cardStyle(2)">
      <div class="lc-row">
    ...
    <div class="life-card pet-card" :style="cardStyle(3)">
      <div class="lc-row">
...
// 多彩卡片配色（对齐首页：金黄/浅橙棕/深红棕/灰紫/深灰绿 循环）
const PALETTE = [
  { bg: 'linear-gradient(135deg,#C4923A,#A8761F)', bd: '#7E5413' },
  { bg: 'linear-gradient(135deg,#C9956C,#A9743F)', bd: '#7C5026' },
  { bg: 'linear-gradient(135deg,#9B4A3E,#7A342B)', bd: '#5C241D' },
  { bg: 'linear-gradient(135deg,#8B8B90,#6E6E73)', bd: '#4F4F53' },
]
...
.lh-illus { font-size: 30px; }
.lh-title { font-size: 20px; font-weight: 700; color: #fff; margin-top: 6px; text-shadow: 0 1px 2px rgba(0,0,0,.3); }
.lh-sub { font-size: 12px; color: #BBB; margin-top: 6px; }

.section-label { display: flex; align-items: baseline; gap: 8px; margin: 18px 18px 10px; }
.section-en { font-size: 11px; color: #FF7B2C; letter-spacing: 1px; font-weight: 600; }
...
.life-card {
  border-radius: 16px;
  border: 3px solid #7E5413;
  box-shadow: inset 0 1px 0 rgba(255,255,255,.20), 0 3px 10px rgba(0,0,0,.35);
  padding: 16px;
  color: #fff;
}
...
.lc-btn { background: rgba(255,255,255,.18); border: 1px solid rgba(255,255,255,.5); color: #fff; }
...
.lc-submit { background: #fff; color: #333; font-weight: 700; border: none; margin-top: 4px; }
...
.rec-cancel { background: transparent; border-color: rgba(255,255,255,.6); color: #fff; }
.rec-st { color: #FFF3D6; }

.life-foot { text-align: center; font-size: 11px; color: #777; margin: 22px 16px 0; }
</style>
